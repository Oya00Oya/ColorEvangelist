...
class DilatedConvolution2D(link.Chain):
    ...
    def _setup_convolution(self, layer):
        ...
        func = dilated_convolution_2d.DilatedConvolution2D(
            n_in, n_out, ksize, stride, pad, dilate=dilate, nobias=not bias_term,
...
def dilated_conv_function(id):
    conv = DilatedConvolution2D(NET_PATH + id + '.caffemodel')

    def forward(x):
        return conv({'data': x}, outputs=['ConvNdBackward1'])[0]
